...
class BasePayment:
            ...
            for k, v in patten.items():
                if v.get('required') and (v.get('type') is str):
                    if parameters.get(k) is None:
                        ...
                        raise Exception(
                            '%s max langth is %d.' % (k, v.get('max', Decimal('Infinity')))
                        )
    ...
    def generate_check_value(self, params):
        ...
        encoding_lst.append(''.join([f'{key}={value}&' for key, value in ordered_params.items()]))
...
class ExtendFunction(BasePayment):
    def gen_html_post_form(self, action, parameters):
        ...
        for k, v in parameters.items():
            html += '<input type="hidden" name="' + str(k) + '" value="' + str(v) + '" />'
...
class CreateOrder(BasePayment):
    ...
    def create_order(self, client_parameters):
        ...
        if choose_payment == ChoosePayment['ALL'] or choose_payment == ChoosePayment['ATM']:
            payment_extend_parameters = self.create_default_dict(self.__ATM_EXTEND_PARAMETERS)
            self.__check_pattern.append(self.__ATM_EXTEND_PARAMETERS)
            # 合併
            default_parameters = super().merge(default_parameters, payment_extend_parameters)
        ...
        # 使用 ALL 或 Credit 付款方式
        if choose_payment == ChoosePayment['ALL'] or choose_payment == ChoosePayment['Credit']:
            payment_extend_parameters = self.create_default_dict(self.__CREDIT_EXTEND_PARAMETERS_1)
        ...
        if choose_payment == ChoosePayment['Credit']:
            payment_extend_parameters = self.create_default_dict(self.__CREDIT_EXTEND_PARAMETERS_2)
        ...
        if (
            client_parameters.get('ChooseSubPayment') == ChooseSubPayment['WebATM']['TACHONG']
            or client_parameters.get('ChooseSubPayment') == ChooseSubPayment['WebATM']['SINOPAC']
        ):
            raise Exception('ChooseSubPayment is not supported with TACHONG or SINOPAC.')
        ...
        if choose_payment == ChoosePayment['ALL'] or choose_payment == ChoosePayment['Credit']:
            ...
            if credit_extend_parameters:
                default_parameters = super().merge(default_parameters, credit_extend_parameters)

        # 看看 client.dict 有無 invoice='Y'
        # 有的話 new.required.dict 合併 invoice.dict
        if client_parameters.get('InvoiceMark') == 'Y':
            invoice_parameters = self.create_default_dict(self.__INVOICE_EXTEND_PARAMETERS)
            ...
            if customer_identifier and (len(customer_identifier) != 8):
                raise Exception('CustomerIdentifier have to fill fixed length of 8 digits.')
            ...
            if customer_identifier and (client_parameters.get('Print') == '0'):
                raise Exception('Print have to fill "1", when CustomerIdentifier have value.')
            # 統一編號 CustomerIdentifier 有值時，Donation 要為 '0'，否則會出現錯誤訊息
            if customer_identifier and (client_parameters.get('Donation') == '1'):
                raise Exception('Donation have to fill "0", when CustomerIdentifier have value.')
        ...
        self.final_merge_parameters = super().merge(default_parameters, client_parameters)
...
class OrderSearchPeriodic(BasePayment):
    ...
    def order_search_period(self, action_url=__url, client_parameters={}):
        ...
        # 先用 required.dict 設定預設值並產生新 new.required.dict
        default_parameters = dict()
        default_parameters = self.create_default_dict(self.__ORDER_SEARCH_PERIODIC_PARAMETERS)
...
class CreditDoAction(BasePayment):
    ...
    def credit_do_action(self, action_url=__url, client_parameters={}):
        ...
        # 先用 required.dict 設定預設值並產生新 new.required.dict
        default_parameters = dict()
        default_parameters = self.create_default_dict(self.__CREDIT_DO_ACTION_PARAMETERS)
...
class DownloadMerchantBalance(BasePayment):
    ...
    def download_merchant_balance(self, action_url=__url, client_parameters={}):
        ...
        # 先用 required.dict 設定預設值並產生新 new.required.dict
        default_parameters = dict()
        default_parameters = self.create_default_dict(self.__DOWNLOAD_MERCHANT_BALANCE_PARAMETERS)
...
class SearchSingleTransaction(BasePayment):
    ...
    def search_single_transaction(self, action_url=__url, client_parameters={}):
        ...
        # 先用 required.dict 設定預設值並產生新 new.required.dict
        default_parameters = dict()
        default_parameters = self.create_default_dict(self.__SEARCH_SINGLE_TRANSACTION_PARAMETERS)
